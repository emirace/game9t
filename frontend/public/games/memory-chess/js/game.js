...
var themesSettings = [
	{
		boards:[
			'assets/board_01_a.png',
			'assets/board_01_b.png',
		],
	...
	{
		boards:[
			'assets/board_02_a.png',
			'assets/board_02_b.png',
		],
	...
	{
		boards:[
			'assets/board_03_a.png',
			'assets/board_03_b.png',
		],
		colors:['RED','BLUE','YELLOW','BLACK','WHITE','GREEN'],
	...
	{
		boards:[
			'assets/board_04_a.png',
			'assets/board_04_b.png',
		],
		colors:['RED','BLUE','YELLOW','BLACK','WHITE','GREEN'],
	...
	{
		boards:[
			'assets/board_05_a.png',
			'assets/board_05_b.png',
		],
		colors:['RED','BLUE','YELLOW','PURPLE','WHITE','GREEN'],
...
var textDisplay = {
	...
	winRules:["Largest number of pieces","4 same color pieces","6 piece of each colors"],
	exitTitle:'EXIT GAME',
...
var shareMessage = '[SCORE] pieces is mine new highscore on Memory Chess game! Try it now!'; //social share score message
...
var gameData = {paused:true, player:0, lastPlayer:0, players:0, themeIndex:0, sizeIndex:0, ai:true, complete:false, names:[], reveals:[]};
...
function toggleTotalPlayers(con){
	if(con){
		gameData.players++;
		gameData.players = gameData.players > gameData.memorychess.maxPlayers ? gameData.memorychess.maxPlayers : gameData.players;
	}else{
		gameData.players--;
		gameData.players = gameData.players < gameData.memorychess.minPlayers ? gameData.memorychess.minPlayers : gameData.players;
	}
...
function toggleBeads(con){
	if(con){
		gameData.sizeIndex++;
		gameData.sizeIndex = gameData.sizeIndex > gameData.memorychess.sizes.length-1 ? gameData.memorychess.sizes.length-1 : gameData.sizeIndex;
	}else{
		gameData.sizeIndex--;
...
function toggleTheme(con){
	if(con){
		gameData.themeIndex++;
		gameData.themeIndex = gameData.themeIndex > themesSettings.length-1 ? themesSettings.length-1 : gameData.themeIndex;
	}else{
		gameData.themeIndex--;
...
function updateCardsOption(){
	...
	if ( typeof initSocket == 'function' && multiplayerSettings.enable && socketData.online) {
		if(socketData.host){
			postSocketUpdate('updateoptions', {sizeIndex:gameData.sizeIndex, winMode:gameData.winMode, themeIndex:gameData.themeIndex, option:gameData.lastOption}, true);
		}
	}
}
...
function displayBoardOptions(){
	...
	if(gameData.lastThemeIndex != gameData.themeIndex){
		...
		var randomPiece = [];
		for(var c=0; c<themesSettings[gameData.themeIndex].pieceTop.length; c++){
			randomPiece.push(c);
		}
		shuffle(randomPiece);
		...
		for(var n=0; n<randomPiece.length; n++){
			var newPiece = new createjs.Bitmap(loader.getResult('pieceTop'+gameData.themeIndex+'_'+randomPiece[n]));
			centerReg(newPiece);
			newPiece.scaleX = newPiece.scaleY = .7;
...
function preparePlayers(){
	...
	for(var n=0; n<gameData.players; n++){
		...
		$.players[n].addChild(bgPlayer, bgPlayerH, newPlayerName, newPlayerScore, newPlayerScoreType, newPlayerStatus);
...
function buildBoard(){
	...
	itemDiceH = new createjs.Bitmap(loader.getResult('diceHighlight'+gameData.themeIndex));
	centerReg(itemDiceH);
	...
	if(gameData.sizeIndex == 0){
		var bgBoard = new createjs.Bitmap(loader.getResult('board'+gameData.themeIndex+'_'+0));
		centerReg(bgBoard);
		...
		for(var n=0; n<chessData.total; n++){
			$.chess[chessIndex] = new createjs.Bitmap(loader.getResult('hole'+gameData.themeIndex));
			centerReg($.chess[chessIndex]);

			$.chess['highlight'+chessIndex] = new createjs.Bitmap(loader.getResult('holeHighlight'+gameData.themeIndex));
			...
			$.chess[chessIndex].y = $.chess['highlight'+chessIndex].y = chessPos.y * .6;

			gameData.pieces.push($.chess[chessIndex]);
			boardDesignContainer.addChild($.chess[chessIndex], $.chess['highlight'+chessIndex]);
		...
		var bgBoard = new createjs.Bitmap(loader.getResult('board'+gameData.themeIndex+'_'+1));
		centerReg(bgBoard);
		boardDesignContainer.addChild(bgBoard);
		...
		for(var r=0; r<chessData.row; r++){
			for(var c=0; c<chessData.column; c++){
				...
				if(!skipCon){
					...
					$.chess[chessIndex].x = $.chess['highlight'+chessIndex].x = chessData.x;
					$.chess[chessIndex].y = $.chess['highlight'+chessIndex].y = chessData.y;
	...
	for(var n=0; n<themesSettings[gameData.themeIndex].pieceTop.length; n++){
...
function setupChess(){
	...
	for(var n=0; n<gameData.pieces.length; n++){
		...
		newMask.x = $.chess[n].x;
		newMask.y = $.chess[n].y;
		newMask.graphics.beginFill('red').mt(-19,-200).lt(19,-200).lt(19,0).qt(0,18,-19,0);
		
		var pieceIndex = gameData.pieceColors[n];
		var pieceTop = new createjs.Bitmap(loader.getResult('pieceTop'+gameData.themeIndex+'_'+pieceIndex));
		pieceTop.regX = themesSettings[gameData.themeIndex].pieceTop[pieceIndex].regX;
		pieceTop.regY = themesSettings[gameData.themeIndex].pieceTop[pieceIndex].regY;
		pieceTop.x = pieceTop.oriX = $.chess[n].x;
		pieceTop.y = pieceTop.oriY = $.chess[n].y - themesSettings[gameData.themeIndex].pieceTop[pieceIndex].height;
		pieceTop.mask = newMask;
		pieceTop.chessIndex = n;
		pieceTop.pieceIndex = pieceIndex;
		boardIconContainer.addChild(pieceTop);

		var pieceBottom = new createjs.Bitmap(loader.getResult('pieceBottom'+gameData.themeIndex+'_'+pieceIndex));
		pieceBottom.regX = themesSettings[gameData.themeIndex].pieceBottom[pieceIndex].regX;
		pieceBottom.regY = themesSettings[gameData.themeIndex].pieceBottom[pieceIndex].regY;
		pieceBottom.x = pieceBottom.oriX = $.chess[n].x;
		pieceBottom.y = pieceBottom.oriY = $.chess[n].y - themesSettings[gameData.themeIndex].pieceBottom[pieceIndex].height;
		...
		pieceTop.addEventListener("click", function(evt) {
			...
			if ( typeof initSocket == 'function' && multiplayerSettings.enable && socketData.online) {
				postSocketUpdate('showpiece', {chessIndex:evt.target.chessIndex, pieceIndex:evt.target.pieceIndex}, true);
			}
		});
...
function setColorFilter(obj, r, g, b){
	obj.filters = [
		new createjs.ColorFilter(0,0,0,1,r,g,b,0)
	];
	var cacheW = 100;
	var cacheH = 300;
...
function animateChess(){
	...
	for(var n=0; n<gameData.pieces.length; n++){
		...
		$.chess[chessIndex].pieceTarget.x = $.chess[chessIndex].pieceTarget.x + randomIntFromInterval(-50,50);
		$.chess[chessIndex].pieceTarget.y = $.chess[chessIndex].pieceTarget.y + randomIntFromInterval(50,100);
		
		delayNum = randomIntFromInterval(3,6) * .1;
		TweenMax.to($.chess[chessIndex].pieceTarget, gameSettings.showColorPickSpeed, {delay:delayNum, rotation:0, x:$.chess[chessIndex].x, y:$.chess[chessIndex].y - 120, ease:'Expo.easeOut', onStart:function(){
			var randomPick = randomIntFromInterval(1,3);
			playSound('soundPick'+randomPick);
		},
		onComplete:animateChessComplete, onCompleteParams:[chessIndex]});
	}
}

function animateChessComplete(n){
	var rotateNum = randomBoolean() == true ? 180 : -180;
	var delayNum = gameSettings.showColorTime + (randomIntFromInterval(0,3) * .1);
	TweenMax.to($.chess[n].pieceTarget, gameSettings.showColorDownSpeed, {delay:delayNum, rotation:rotateNum, y:$.chess[n].pieceTarget.oriY, onStart:function(){
		...
		var delayNum = randomIntFromInterval(0,.3) * .1;
		TweenMax.to($.chess[n].pieceTarget, gameSettings.showColorDownSpeed, {delay:delayNum, x:$.chess[n].pieceTarget.oriX, y:$.chess[n].pieceTarget.oriY, onStart:function(){
...
function showPiece(chessIndex, pieceIndex){
	...
	TweenMax.to($.chess[chessIndex].pieceTarget, gameSettings.pickSpeed, {y:oriY - 80, onComplete:function(){
		TweenMax.to($.chess[chessIndex].pieceTarget, gameSettings.pickDelay, {onComplete:function(){
			if(gameData.diceIndex == pieceIndex){
				...
				playSound('soundPlace');
				TweenMax.to($.chess[chessIndex].pieceTarget, gameSettings.backDownSpeed, {y:oriY, onComplete:function(){
					//end turn
...
function showCorrectPiece(chessIndex, pieceIndex){
	...
	playSound('soundPlace');
	TweenMax.to($.chess[chessIndex].pieceTarget, gameSettings.correctSpeed, {rotation:rotateNum, y:oriY-50, onComplete:function(){
		...
		TweenMax.to($.chess[chessIndex].pieceTarget, gameSettings.correctSpeed, {y:$.chess[chessIndex].pieceTarget.oriY, onComplete:function(){
			checkBoardWin();
		}});
	}});
}
...
function showPieceComplete(check){
	if ( typeof initSocket == 'function' && multiplayerSettings.enable && socketData.online) {
		postSocketUpdate('showpiececomplete', {index:socketData.gameIndex, check:check});
	}else{	
		if(check){
...
function showPlayerPieces(player){
	for(var n=0; n<gameData.pieces.length; n++){
		if($.chess[n].pieceBottom.visible && !$.chess[n].animating){
			...
			TweenMax.to($.chess[n].pieceBottom, .2, {y:$.chess[n].pieceBottom.oriY, overwrite:true});
		}
	}

	for(var n=0; n<$.players[player].pieces.length; n++){
		var chessIndex = $.players[player].pieces[n];
		if($.chess[chessIndex].pieceBottom.visible && !$.chess[chessIndex].animating){
			$.chess[chessIndex].pieceBottomH.visible = false;
			TweenMax.to($.chess[chessIndex].pieceBottom, .5, {y:$.chess[chessIndex].pieceBottom.oriY - 50, ease:'Expo.easeOut', overwrite:true, onComplete:showPlayerPiecesComplete, onCompleteParams:[chessIndex]});
			TweenMax.to($.chess[chessIndex].pieceBottomH, .5, {alpha:.3, y:$.chess[chessIndex].pieceBottom.oriY - 50, ease:'Expo.easeOut', overwrite:true});
		}
	}
}

function showPlayerPiecesComplete(chessIndex){
	TweenMax.to($.chess[chessIndex].pieceBottom, .5, {y:$.chess[chessIndex].pieceBottom.oriY, ease:'Expo.easeIn', overwrite:true});
	TweenMax.to($.chess[chessIndex].pieceBottomH, .5, {alpha:0, y:$.chess[chessIndex].pieceBottom.oriY, ease:'Expo.easeIn', overwrite:true, onComplete:function(){
		$.chess[chessIndex].pieceBottomH.visible = false;
	}});
}
	...
	if ( typeof initSocket == 'function' && multiplayerSettings.enable && socketData.online) {
		if(isPlayer){
			if(gameData.rollCon){
				...
				gameData.rollCon = false;
				proceedRollDice();
				postSocketUpdate('proceedrolldice', {diceIndex:gameData.diceIndex}, true);
			}
		}
	}else{
		var isPlayer = checkIsPlayer(gameData.player);
...
function proceedRollDice(){
	...
	var rollPos = [];
	var pos = {x:itemDice.oriX, y:itemDice.oriY, startX:itemDice.oriX, startY:itemDice.oriY, radiusX:0, radiusY:0};
	var radiusX = 20;
	var radiusY = 20/1.5;
	var radiusW = 600;
	var radiusH = 100;
	...
	TweenMax.to(itemDice, 1, {bezier:{type:"thru", values:rollPos, curviness:2, autoRotate:false}, ease:Linear.easeNone, overwrite:true, onComplete:rollDiceComplete});
}
...
function rollDiceNext(){
	...
	if(isPlayer){
		...
		if ( typeof initSocket == 'function' && multiplayerSettings.enable && socketData.online) {
			...
			//ai
			TweenMax.to(boardContainer, gameSettings.aiThinkSpeed, {onComplete:function(){
				aiMove();
			}});
		}
	}
...
function aiMove(){
	...
	if(gameData.reveals.length > 0){
		for(var n=0; n<gameData.reveals.length; n++){
			...
			if($.chess[chessIndex].pieceTop.visible && $.chess[chessIndex].pieceTop.pieceIndex == gameData.diceIndex){
...
function displayPlayerTurn(){
	...
	if(gameData.ai && gameData.player != 0){
		TweenMax.to(boardContainer, .5, {onComplete:function(){
			rollDice(false);
		}});
	}else{
		if(isPlayer){
...
function animateBlink(obj){
	...
	TweenMax.to(obj, tweenSpeed, {alpha:1, overwrite:true, onComplete:function(){
		TweenMax.to(obj, tweenSpeed, {alpha:.5, overwrite:true, onComplete:animateBlink, onCompleteParams:[obj]});
	}});
}

function aniamtePlayerFocus(obj){
	TweenMax.to(obj, .2, {scaleX:1.3, scaleY:1.3, ease:Sine.easeIn,  overwrite:true, onComplete:function(){
...
function updatePlayerScore(player){
	if(gameData.settings.winMode == 0){
		...
		$.players[player].playerScoreType.text = textDisplay.scoreType[gameData.settings.winMode];
	}else if(gameData.settings.winMode == 1){
		...
		if(playerData.scores[player] == 0){
			...
			$.players[player].playerScoreType.text = themesSettings[gameData.themeIndex].colors[currentColor];
		}
	}else if(gameData.settings.winMode == 2){
		$.players[player].playerScore.text = 'x' + playerData.scores[player];
		if(playerData.scores[player] == 0){
...
function showInstruction(type){
	...
	instructionContainer.alpha = 0;
	TweenMax.to(instructionContainer, .5, {alpha:1, overwrite:true, onComplete:function(){
		TweenMax.to(instructionContainer, .5, {delay:delayNum, alpha:0, overwrite:true, onComplete:function(){
...
function showEndStatus(){
	...
	if(gameData.settings.winMode == 0){
		if(isPlayer){
			...
			var winText = textDisplay.playerWin.replace('[NAME]', $.players[topScoreIndex].playerName.text);
		}
	}else{
