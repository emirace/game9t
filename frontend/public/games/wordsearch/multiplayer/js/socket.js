...
var multiplayerSettings = {
  ...
  rejoinRoom: false, //button replay at result screen to rejoin current rooms (select public rooms only)
};
...
function initSocket(game) {
  ...
  socket = io("http://213.130.147.200:5000", {
    autoConnect: true,
    extraHeaders: {
...
function updateSocketRooms(rooms) {
  ...
  for (var n = 0; n < rooms.length; n++) {
    var totalUsers =
      rooms[n].users.length > 0 ? " (" + rooms[n].users.length + ")" : "";
    var playing =
      rooms[n].play == true
        ? "(" + textMultiplayerDisplay.playing + ")"
        : totalUsers;
...
function switchSocketRoomContent(con) {
  ...
  if (con == "lists") {
    ...
    if (socketData.public) {
      ...
      $("#enterRoomTitle").html(
        textMultiplayerDisplay.privateRoomTitle + " (" + socketData.room + ") :"
      );
    }
...
function updateSocketFindPlayer() {
  ...
  if (multiplayerSettings.findPlayerTimer && socketData.showTimer) {
    ...
    loadingText += textMultiplayerDisplay.findingPlayerTimer.replace(
      "[TIMER]",
      seconds
    );
  }
  ...
  socketData.loaderIndex =
    socketData.loaderIndex > 3 ? 1 : socketData.loaderIndex;
}
