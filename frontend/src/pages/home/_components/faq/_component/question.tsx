...
export const Question: React.FC<IQuestion> = ({ content, title }) => {
  ...
  return (
    <div
      ...
      className="relative bg-dark p-4 rounded-md transform transition-transform duration-300 ease-in-out"
    >
      <div className=" font-jua">{title}</div>
      {showContent && <div className="text-sm">{content}</div>}
      <div className="bg-cream p-1 absolute top-3 right-3 rounded-sm">
        <img
